...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const routes: Record<string, string> = { ADMIN: "/admin", AGENT: "/agent", CLIENT: "/portal" };
  ...
  return (
    <div className="flex flex-1 min-h-screen bg-gradient-to-br from-slate-100 via-blue-50 to-indigo-100">
      {/* Left side — branding */}
      <div className="hidden lg:flex lg:w-1/2 items-center justify-center p-12">
        <div className="max-w-md text-center">
          <div className="flex h-16 w-16 mx-auto items-center justify-center rounded-2xl bg-indigo-600 shadow-xl shadow-indigo-200 mb-6">
            <svg viewBox="0 0 24 24" fill="none" className="h-9 w-9 text-white" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 3C7 3 3 7 3 12s4 9 9 9 9-4 9-9-4-9-9-9z" />
              <path strokeLinecap="round" strokeLinejoin="round" d="M8 12h8M12 8v8" />
            </svg>
          </div>
          <h2 className="text-3xl font-extrabold text-slate-800 mb-3">Servando</h2>
          <p className="text-lg text-slate-600 mb-8">שירות מענה טלפוני חכם לעסקים קטנים</p>
          <div className="space-y-4 text-start">
            {[
              { icon: "📞", text: "מענה מקצועי בשם העסק שלך" },
              { icon: "📋", text: "תיעוד ומעקב בזמן אמת" },
              { icon: "🤖", text: "AI לסיכומים ופולו-אפ אוטומטי" },
              { icon: "📊", text: "דשבורד עם BI ודוחות מתקדמים" },
            ].map((f) => (
              <div key={f.text} className="flex items-center gap-3 rounded-xl bg-white/70 backdrop-blur px-4 py-3 shadow-sm">
      ...
      <div className="flex flex-1 items-center justify-center px-4 py-12">
        <div className="w-full max-w-sm">
          ...
          <div className="mb-8 text-center lg:hidden">
            ...
            <p className="mt-2 text-sm text-slate-500">שירות מענה טלפוני חכם</p>
          </div>

          {/* Card */}
          <div className="rounded-2xl bg-white px-8 py-8 shadow-xl shadow-slate-200/60 border border-slate-100">
            <h1 className="mb-1 text-xl font-semibold text-slate-800 text-center">כניסה למערכת</h1>
            <p className="mb-6 text-sm text-slate-400 text-center">הזן את פרטי החשבון שלך</p>
